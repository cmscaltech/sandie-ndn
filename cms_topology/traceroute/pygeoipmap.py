...
def get_lat_lon(ip_list=[], lats=[], lons=[], labels = []):
    """
...
def geoip_lat_lon(gi, ip_list=[], lats=[], lons=[], labels=[]):
    """
...
def generate_map(output, lats=[], lons=[], labels=[], wesn=None):
    """
    ...
    if wesn:
       ...
       # m = Basemap(projection='cyl', resolution='l')
        m = Basemap(width=12000000,height=9000000,projection='lcc',
            resolution=None,lat_1=45.,lat_2=55,lat_0=47,lon_0=-107.)
    m.bluemarble()
    x, y = m(lons, lats)
    m.scatter(x, y, s=2, color='#ff0000', marker='o')
...
def main():
    ...
    if args.format == 'ip':
        ...
        if args.service == 'm':
            ...
            lats, lons, labels = get_lat_lon(ip_list)
    elif args.format == 'csv':
        lats, lons = get_lat_lon_from_csv(args.input)

    generate_map(output, lats, lons, labels, wesn=args.extents)

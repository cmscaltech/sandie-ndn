...
if(NDN_CXX_INCLUDES-NOTFOUND)
  print_message(SEND_ERROR "Looking for ndn-cxx/version.hpp - not found")
  set(NDN_CXX-NOTFOUND TRUE)
else()
  # Set ndn-cxx version
  file(READ "${NDN_CXX_INCLUDES}/version.hpp" ndn-cxx-version-content)
  string(REGEX MATCH
               "NDN_CXX_VERSION_STRING [\"]([0-9]+)\\.([0-9]+)\\.([0-9]+)[\"]"
               NDN_CXX-VERSION
               ${ndn-cxx-version-content})
  string(REPLACE "NDN_CXX_VERSION_STRING "
                 ""
                 NDN_CXX-VERSION
                 ${NDN_CXX-VERSION})
  string(REPLACE "\""
  ...
  # Set ndn-cxx path to lib directory
...
# Get path to libndn-cxx
find_library(NDN_CXX_LIB ndn-cxx HINTS "/usr/" "/usr/local/")

if(NDN_CXX_LIB-NOTFOUND)
  print_message(SEND_ERROR "Looking for libndn-cxx - not found")
  set(NDN_CXX-NOTFOUND TRUE)
else()
...
if(NDN_CXX_LIB-NOTFOUND)
  if(ndn-cxx_FIND_REQUIRED)
    message(SEND_ERROR "ndn-cxx package not found")
  else()
    print_message(SEND_ERROR "ndn-cxx package not found")
  endif()
else()
...
if(ndn-cxx_FIND_VERSION_EXACT)
  if(NOT NDN_CXX-VERSION VERSION_EQUAL ndn-cxx_FIND_VERSION)
    message(FATAL_ERROR "ndn-cxx package version: " ${ndn-cxx_FIND_VERSION}
                       " not found")
  endif()
else()
  if(NDN_CXX-VERSION VERSION_LESS ndn-cxx_FIND_VERSION)
    message(FATAL_ERROR "Require at least ndn-cxx version "
                        ${ndn-cxx_FIND_VERSION})
  endif()
endif()
